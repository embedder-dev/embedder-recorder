 ...
 *
 * Custom tracing_user.h for the Embedder tracing module.
 *
 * Based on Zephyr's stock subsys/tracing/user/tracing_user.h but
 * adapted for force-inclusion via -include.  The key change: we do NOT
 * include <zephyr/kernel.h> because this header is force-included into
 * every translation unit, including Zephyr build-system helpers
 * (e.g. heap_constants.c) where kernel.h creates a circular dependency
 * with generated headers.  Instead we use lightweight forward
 * declarations and <zephyr/sys/clock.h> for k_timeout_t.
 *
 * Uses the same include guard (_TRACE_USER_H) as Zephyr's stock header,
 * preventing the stock header from being loaded.
 *
 * When CONFIG_EMBEDDER_TRACE_SCHEDULING_ONLY is NOT defined, the
 * semaphore, mutex, and timer macros are wired to our trace functions
...
#include <zephyr/sys/clock.h>  /* k_timeout_t — lightweight, no kernel.h dep */
#include <zephyr/sys/slist.h>  /* sys_slist_t — for GPIO fire_callbacks */
#include <stdint.h>

/* Forward declarations — only pointers used in prototypes below */
struct k_thread;
struct k_timer;
struct k_sem;
struct k_mutex;
...
/*
 * Local GPIO typedefs — mirrors Zephyr's own tracing_user.h so that
 * Zephyr's tracing_user.c compiles without pulling in drivers/gpio.h.
 * Duplicate identical typedefs are permitted in C11+.
 */
...
/* ── _user() function declarations (override __weak stubs) ──────── */
...
void sys_trace_sys_init_exit_user(const struct init_entry *entry,
...
void sys_trace_sys_init_exit(const struct init_entry *entry,
			     int level, int result);

/* Timer wrapper declarations */
void sys_trace_timer_init(struct k_timer *timer);
void sys_trace_timer_start(struct k_timer *timer, k_timeout_t duration,
			   k_timeout_t period);
void sys_trace_timer_stop(struct k_timer *timer);
void sys_trace_timer_status_sync_enter(struct k_timer *timer);
void sys_trace_timer_status_sync_blocking(struct k_timer *timer,
					  k_timeout_t timeout);
...
/* GPIO _user() declarations */
void sys_trace_gpio_pin_interrupt_configure_enter_user(const struct device *port,
						       gpio_pin_t pin, gpio_flags_t flags);
void sys_trace_gpio_pin_interrupt_configure_exit_user(const struct device *port,
						      gpio_pin_t pin, int ret);
void sys_trace_gpio_pin_configure_enter_user(const struct device *port,
					     gpio_pin_t pin, gpio_flags_t flags);
...
void sys_trace_gpio_pin_get_config_enter_user(const struct device *port,
					      gpio_pin_t pin, int ret);
void sys_trace_gpio_pin_get_config_exit_user(const struct device *port,
					     gpio_pin_t pin, int ret);
void sys_trace_gpio_port_get_raw_enter_user(const struct device *port,
					    gpio_port_value_t *value);
void sys_trace_gpio_port_get_raw_exit_user(const struct device *port, int ret);
void sys_trace_gpio_port_set_masked_raw_enter_user(const struct device *port,
						   gpio_port_pins_t mask,
						   gpio_port_value_t value);
void sys_trace_gpio_port_set_masked_raw_exit_user(const struct device *port, int ret);
void sys_trace_gpio_port_set_bits_raw_enter_user(const struct device *port,
						 gpio_port_pins_t pins);
...
void sys_trace_gpio_port_toggle_bits_enter_user(const struct device *port,
						gpio_port_pins_t pins);
...
void sys_trace_gpio_fire_callbacks_enter_user(sys_slist_t *list,
					      const struct device *port,
					      gpio_pin_t pins);
void sys_trace_gpio_fire_callback_user(const struct device *port,
				       struct gpio_callback *callback);

/* RTIO wrapper declarations */
...
void sys_trace_rtio_sqe_acquire_exit(const struct rtio *r,
				     const struct rtio_sqe *sqe);
void sys_trace_rtio_sqe_cancel(const struct rtio_sqe *sqe);
void sys_trace_rtio_cqe_submit_enter(const struct rtio *r, int result,
				     uint32_t flags);
void sys_trace_rtio_cqe_submit_exit(const struct rtio *r);
void sys_trace_rtio_cqe_acquire_enter(const struct rtio *r);
void sys_trace_rtio_cqe_acquire_exit(const struct rtio *r,
				     const struct rtio_cqe *cqe);
void sys_trace_rtio_cqe_release(const struct rtio *r,
				const struct rtio_cqe *cqe);
void sys_trace_rtio_cqe_consume_enter(const struct rtio *r);
void sys_trace_rtio_cqe_consume_exit(const struct rtio *r,
				     const struct rtio_cqe *cqe);
...
/* Embedder sem/mutex trace function declarations */
...
#define sys_port_trace_k_thread_sched_ready(thread) \
	sys_trace_thread_sched_ready(thread)
 ...
 * ── ISR / Idle Macros ────────────────────────────────────────────
 */
#define sys_port_trace_isr_enter() sys_trace_isr_enter()
#define sys_port_trace_isr_exit() sys_trace_isr_exit()
#define sys_port_trace_idle() sys_trace_idle()
...
/*
 * ── Remaining kernel objects — all no-ops ────────────────────────
 * Copied from Zephyr 4.x stock tracing_user.h for API compatibility.
 */
...
#define sys_port_trace_socket_getsockopt_exit(sock, level, optname, optval, \
					     optlen, ret)
#define sys_port_trace_socket_setsockopt_enter(sock, level, optname, optval, \
					      optlen)
...
#define sys_port_trace_gpio_port_get_direction_enter(port, map, inputs, \
						    outputs) \
