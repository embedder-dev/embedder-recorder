 ...
 *
 * This header is force-included via -include before any Zephyr code
 * compiles. It uses the same include guard (_TRACE_USER_H) as
 * Zephyr's stock subsys/tracing/user/tracing_user.h, preventing
 * the stock header from being loaded.
 *
 * When CONFIG_TRACING_USER is selected in prj.conf, Zephyr's
 * tracing.h dispatches to "tracing_user.h" — our force-included
 * version is found first.
 *
 * The sys_port_trace_* macros defined here wire kernel trace points
 * to our implementations in trace_kernel.c.
 */
...
/*
 * ── Weak _user() function declarations ───────────────────────────
 *
 * These override the __weak stubs in Zephyr's tracing_user.c.
 * Zephyr's wrapper functions call these.
 */
...
void sys_trace_sys_init_enter_user(const struct init_entry *entry,
				   int level);
void sys_trace_sys_init_exit_user(const struct init_entry *entry,
				  int level, int result);

/*
 * Zephyr's tracing_user.c wrapper functions — declared here so the
 * macros below can reference them. These are defined in
 * zephyr/subsys/tracing/user/tracing_user.c.
 */
...
/* Timer — called directly from macros (no Zephyr wrappers in this NCS) */
 ...
 * ── Timer Macros ─────────────────────────────────────────────────
 *
 * Called directly from macros — no Zephyr wrappers in this NCS version.
 */
#if !defined(CONFIG_EMBEDDER_TRACE_SCHEDULING_ONLY)
...
/*
 * ── Untraced Kernel Objects ──────────────────────────────────────
 * All remaining sys_port_trace_* macros expand to nothing.
 */
...
#define sys_port_trace_socket_recvfrom_enter(sock, max_len, flags, addr, \
					    addrlen)
...
/* GPIO — all no-ops (no GPIO tracing in our module) */
